...
from utils import verify, xl_utils
...
class EmployeesPage(BrowsePage):
    ...
    def verify_page_title(self):
        verify.verify_element_text(
            self.driver, self.page_title_xpath, "Employee Management"
        )
...
class AddEmployeeModal(EmployeesPage):
    ...
    edit_photo_frame_xpath = (
        By.XPATH,
        "//img-crop[@id='employeePhotoPreview']//canvas",
    )
    ...
    def enter_joined_date(self, joined_date: str):
        ...
        self.fill_in_with_value(
            self.joined_date_field_xpath, joined_date.strftime("%Y-%m-%d")
        )
    ...
    def enter_location(self, location: str):
        ...
        self.get_element(
            (
                By.XPATH,
                f"//div[@class='custom-dropdown-item-inner-container']"
                f"//span[text()='{location}']",
            )
        ).click()
        ...
        for i in range(1, columns + 1):
            if xl_utils.read_data(file, sheet_name, 1, i) == "first_name":
                self.enter_first_name(
                    xl_utils.read_data(file, sheet_name, row_index, i)
                )
            elif xl_utils.read_data(file, sheet_name, 1, i) == "middle_name":
                self.enter_middle_name(
                    xl_utils.read_data(file, sheet_name, row_index, i)
                )
